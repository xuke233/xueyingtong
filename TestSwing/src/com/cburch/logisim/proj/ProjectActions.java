...
public class ProjectActions {
	...
	//component 是一个具有图形表示能力的对象，可在屏幕上显示，
	// 并可与用户进行交互。典型图形用户界面中的按钮、复选框和滚动条都是组件示例。
	public static void doOpen(Component parent, Project baseProject) {
		JFileChooser chooser;//选择文件需要的类
		if (baseProject != null) {
		...
		File selected = chooser.getSelectedFile();//选中文件
		if (selected != null) {
	...
	public static Project doOpen(Component parent,
		...
		Project proj = Projects.findProjectFor(f);//在已打开的project里面看看有没有
		Loader loader = null;
		if (proj != null) {
		...
		try {
			LogisimFile lib = null;//出错在这一句
			if (loader != null) {
				lib = loader.openLogisimFile(f);
			}
		...
		System.out.println("doOpen() exit");
		return proj;
	...
	public static Project doOpenT(Component parent,
		...
		try {
			LogisimFile lib = loader.openLogisimFile(f);//出错在这一句
			proj = new Project(lib);
			//AppPreferences.updateRecentFile(f);
		} catch (LoadFailedException ex) {
			return null;
